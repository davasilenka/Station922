...
' {D9A1F3B7-5C28-4F91-A2E3-8B6C4D0E1F5A}
DEFINE_CLSID(CLSID_HTTPCGIPROCESSOR, _
	&hD9A1F3B7, &h5C28, &h4F91, &hA2, &hE3, &h8B, &h6C, &h4D, &h0E, &h1F, &h5A _
)

' {A7B8C9D0-E1F2-4A3B-8C9D-0E1F2A3B4C5D}
DEFINE_IID(IID_IHttpCgiAsyncProcessor, _
	&hA7B8C9D0, &hE1F2, &h4A3B, &h8C, &h9D, &h0E, &h1F, &h2A, &h3B, &h4C, &h5D _
)

' {1E8B7A3C-4D5F-4A2E-9B8C-7D6E5F4A3B2C}
DEFINE_CLSID(CLSID_CGIREADSTDOUTASYNCTASK, _
	&h1E8B7A3C, &h4D5F, &h4A2E, &h9B, &h8C, &h7D, &h6E, &h5F, &h4A, &h3B, &h2C _
)

' {2F9C8D4B-3E5F-4B2D-8C9A-6D7E5F4C3B2A}
DEFINE_CLSID(CLSID_CGIWRITESTDINASYNCTASK, _
	&h2F9C8D4B, &h3E5F, &h4B2D, &h8C, &h9A, &h6D, &h7E, &h5F, &h4C, &h3B, &h2A _
)

' {B8C9D0E1-F2A3-4B4C-9D0E-1F2A3B4C5D6E}
DEFINE_IID(IID_ICgiAsyncIoTask, _
	&hB8C9D0E1, &hF2A3, &h4B4C, &h9D, &h0E, &h1F, &h2A, &h3B, &h4C, &h5D, &h6E _
)
